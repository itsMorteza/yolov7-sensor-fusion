...
def attempt_load_fuse(weights,opt, map_location=None):
    # Loads an ensemble of models weights=[a,b,c] or a single model weights=[a] or weights=a
    with open(opt.hyp) as f:
               hyp = yaml.load(f, Loader=yaml.SafeLoader)  # load hyps
    ckpt = torch.load(weights[0], map_location=map_location)  # load checkpoint
    model = Model(opt.cfg , ch=6, nc=3, anchors=hyp.get('anchors')).to(map_location)  # create
    exclude = ['anchor'] if (opt.cfg or hyp.get('anchors'))  else []  # exclude keys
    state_dict = ckpt['model'].float().state_dict()  # to FP32
    state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect
    model.load_state_dict(state_dict, strict=False)  # load
    # make the model go into fuse mode
