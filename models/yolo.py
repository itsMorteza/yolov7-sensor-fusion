...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None, fusion_type = None):  # model, input channels, number of classes
        ...
        if (self.yaml['fusiontype']!='None'):
            self.fusion = self.yaml['fusiontype']
        ...
        if self.fusion=='MFB' or self.fusion=='BGF' :
            pds=1
            KernelS = 3
        ...
        if isinstance(m, IDetect):
            ...
            if self.fusion:
                GH = 6
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, GH, s, s))])  # forward
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
            self.stride = m.stride
            self._initialize_biases()  # only run once
        ...
        self.info()
        logger.info('')
    def fusion_operation(self, RGB, LIDAR):
        if self.fusion == 'ADD':
            return RGB + LIDAR
            ...
            return self.MFBv4(RGB,LIDAR,0)
        elif self.fusion == 'BGF':
            return self.trad_fusion(RGB,LIDAR,0)
        else:
            return torch.cat([RGB, LIDAR], dim=1)
        

    def forward(self, x, augment=False, profile=False):
        if self.fusion:
            x=self.fusion_operation(x[:,3:],x[:,:3])
